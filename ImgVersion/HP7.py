...
#---------------------config---------------------
add_price_strategy=1 # add price strategy is 1 mean add price with 1000-(price_on_2-price_on_1)>800?1000-(price_on_2-price_on_1):800; 2 means price_on_2+900
second_on_2=48 # for strategy 1 and 2
pushByforce=53 # for strategy 1 and 2
# strategy1 
second_on_1=40 # for strategy 1
getTimeByType=1 # to config which time type will read from(by netwok or by local),2 means local. 1 means from network
...
class HP:
    def __init__(self):
        ...
        self.getTimeByType=getTimeByType
	...
	global price_on_1
	    ...
	    #change to check if the price_on_2 >= price_on_1
            #print "price on 2 is %s,%s" %(price_on_2,price_on_1)
            if price_on_2>=price_on_1 and price_on_1>0 and price_on_2>0:
                self.step="enterPrice"
        #print self.step
        try:
            pass
    ...
    def waitForSubmit(self):
        ...
        global getTimeByType
        if True:
            print "%s:XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" %self.getTimeByType
            ...
            if self.getTimeByType==1:
                if  int(currentSecond)>self.pushByforce-1:
                    ...
                    return
            #########################################################
            #To check if the current second is bigger than pushbyforece(this is time read from this pc) 
            #########################################################
            if self.getTimeByType==2:
                print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
                currentRealSecond=time.strftime('%S',time.localtime(time.time()))
                print "current Real time: %s\nwait time: %s" %(currentRealSecond,0)
                if  int(currentRealSecond)>=self.pushByforce-1:
                    print "current Real time: %s\nwait time: %s" %(currentRealSecond,0)
                    print "Time out(%s)" %self.myPrice
                    self.step="submit"
                    return
